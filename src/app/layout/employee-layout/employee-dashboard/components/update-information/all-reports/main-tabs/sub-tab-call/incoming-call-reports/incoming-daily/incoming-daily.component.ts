...
@Component({
  selector: 'tru-incoming-daily',
  templateUrl: './incoming-daily.component.html',
  styleUrls: ['./incoming-daily.component.scss']
})
export class IncomingDailyComponent implements OnInit, OnDestroy {
  ...
  getDetails = () => {
    ...
    this.getDetailsSubscription = this.empReportService.GetCallReportDetails(this.loggedInuserDetails, this.SelectedDateRange, 'Incoming', 'Daily', null, null).subscribe((response) => {
      if (response && response['data']) {
        this.ChartValues = this.PreparedgraphObject(response['data'][1], 'IncomingDaily', 'Date');
