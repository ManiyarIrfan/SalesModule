...
@Component({
  selector: 'tru-sub-tab-lead-bysource',
  templateUrl: './sub-tab-lead-bysource.component.html',
  styleUrls: ['./sub-tab-lead-bysource.component.scss']
})
export class SubTabLeadBysourceComponent implements OnInit, OnChanges, OnDestroy {
  ...
  getSourceDetail = (data, count: number, heading) => {
    this.leadAnalysisDetailsReportList = [];
    if (data.EmpId && count > 0) {
      this.empDetailModel["ReportName"] = 'BySource';
  ...
  getallLeadDetails = () => {
    ...
    this.isLoading = true;
    this.empReportService.getAllLeadDetails(this.loggedInuserDetails, 'BySource', this.selectedDateRange).subscribe((response) => {
      if (response && response['data'].length > 0) {
        ...
        this.getLeadByStatus.forEach(element => {
          ...
          this.tableHeader.forEach((item, index) => {
            if (item.toLocaleLowerCase() !== 'empid' && item.toLocaleLowerCase() !== 'empname') {
              ...
              total = total + element[item];
            } else {
              item.toLocaleLowerCase() === 'empid' ? this.tableHeader.splice(index, 1) : null;
              item.toLocaleLowerCase() === 'empname' ? this.tableHeader.splice(index, 1) : null;
            }
          });
          element['Total'] = total;
        });
