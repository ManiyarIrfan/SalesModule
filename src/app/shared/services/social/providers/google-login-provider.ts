  ...
  getLoginStatus(): Promise<SocialUser> {
    return new Promise((resolve, reject) => {
      if (this.auth2.isSignedIn.get()) {
        ...
        savedata['CompanyName'] = user.name = profile.getName();
        savedata['Email'] = user.email = profile.getEmail();
        savedata['UserLogo'] = user.photoUrl = profile.getImageUrl();
        savedata['Provider'] = 'Google';
        user.firstName = profile.getGivenName();
        user.lastName = profile.getFamilyName();
        user.authToken = token;
        user.idToken = backendToken;
        // this._signupService.insertSignupDetails(savedata);
        resolve(user);
      } else {
